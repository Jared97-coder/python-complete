...
### Archivo: `laboratorio.py`
Implementa las tres clases/funciones con sus tests:
1. `@retry_with_backoff` - Decorador completo
2. `batch_generator` - Generador por lotes
3. `Timer` - Context manager de temporización

### Tests mínimos requeridos
- [ ] Decorador reintenta correctamente
- [ ] Backoff exponencial funciona
- [ ] Generador produce lotes del tamaño correcto
- [ ] Último lote puede ser menor
- [ ] Timer mide tiempo correctamente
- [ ] Splits funcionan
...
- ✅ Lotes generados eficientemente
...
| Criterio | Puntos |
|----------|--------|
| **Decorador @retry_with_backoff** | 35 pts |
| - Implementación correcta | 10 |
| - Backoff exponencial | 10 |
| - Manejo de excepciones | 8 |
| - Callback funcional | 7 |
| **Generador batch_generator** | 35 pts |
| - Generación de lotes | 10 |
| - Transformación on-the-fly | 10 |
| - Filtrado funcional | 8 |
| - Eficiencia de memoria | 7 |
| **Context Manager Timer** | 30 pts |
| - __enter__/__exit__ correctos | 10 |
| - Splits funcionando | 8 |
| - Reporte detallado | 7 |
| - Manejo de anidamiento | 5 |
| **Total** | **100 pts** |
...
### Bonus 1: Decorador con métricas (+10 pts)
Agrega al decorador la capacidad de registrar métricas (tasa de éxito, tiempos promedio).

### Bonus 2: Generador con progreso (+10 pts)
Agrega barra de progreso al generador usando `tqdm`:
```python
from tqdm import tqdm

def batch_generator_con_progreso(iterable, tamaño_lote):
    for lote in tqdm(batch_generator(iterable, tamaño_lote)):
        yield lote
```

### Bonus 3: Timer con exportación (+10 pts)
Agrega capacidad de exportar métricas del Timer a JSON:
```python
def exportar_json(self, archivo):
    ...
    datos = {
        'nombre': self.nombre,
        'duracion': self.duracion,
        'fases': [vars(f) for f in self.fases]
    }
...
Este laboratorio integra tres conceptos avanzados de Python:
- **Decoradores**: Para modificar comportamiento de funciones (reintentos)
- **Generadores**: Para procesamiento eficiente de grandes volúmenes
- **Context Managers**: Para manejo automático de recursos y mediciones

Estos patrones son fundamentales en código Python profesional y se usan extensivamente en:
- APIs y microservicios (reintentos ante fallos)
- Procesamiento de Big Data (lotes eficientes)
- Monitoreo y observabilidad (temporización y métricas)

**¡Dominar estos conceptos te convertirá en un desarrollador Python senior!** 🚀
